...
const mockResolveTitleTiming = jest.fn(() => 'immediate');
const mockSanitizeModelSpecs = jest.fn((value) => value);
jest.mock('@librechat/api', () => ({
  ...jest.requireActual('@librechat/api'),
  getCloudFrontConfig: (...args) => mockGetCloudFrontConfig(...args),
  resolveBuildInfo: (...args) => mockResolveBuildInfo(...args),
  resolveTitleTiming: (...args) => mockResolveTitleTiming(...args),
  sanitizeModelSpecs: (...args) => mockSanitizeModelSpecs(...args),
}));
...
afterEach(() => {
  ...
  mockResolveTitleTiming.mockReturnValue('immediate');
  mockSanitizeModelSpecs.mockImplementation((value) => value);
...
describe('GET /api/config', () => {
  describe('unauthenticated (no req.user)', () => {
    ...
    it('should preserve branding overrides for unauthenticated requests without leaking post-login fields', async () => {
      mockGetAppConfig.mockResolvedValue(baseAppConfig);
      const app = createApp(null);
