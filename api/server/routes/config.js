...
const { EModelEndpoint, defaultSocialLogins, removeNullishValues } = require('librechat-data-provider');
...
function buildBrandingPayload(appConfig, { includeHelpAndFaqURL = false } = {}) {
...
function buildPostLoginPayload(appConfig) {
  const brandingPayload = buildBrandingPayload(appConfig, { includeHelpAndFaqURL: true });
  ...
  const payload = {
    ...
    helpAndFaqURL: brandingPayload.helpAndFaqURL,
    sharedLinksEnabled,
...
router.get('/', async function (req, res) {
  try {
    ...
    const postLoginPayload = buildPostLoginPayload(appConfig);
